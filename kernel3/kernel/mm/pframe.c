...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
    ...
    new = pframe_get_resident(o, pagenum);
    
    if (NULL == new) {
        if (pageoutd_needed()) {
            pageoutd_wakeup();
            sched_cancellable_sleep_on(&alloc_waitq);
            dbg(DBG_PRINT, "pframe_get\n");
        }
        ...
        *result = new;
        if (NULL == *result) {
            ...
            dbg(DBG_PRINT, "pframe_get\n");
            return pframe_fill(*result);
        }
    }
    else {
        while (pframe_is_busy(new)) {
            new = pframe_get_resident(o, pagenum);
            dbg(DBG_PRINT, "pframe_get\n");
        }
        *result = new;
...
pageoutd_run(int arg1, void *arg2)
{
        while (1) {
                ...
                if (sched_cancellable_sleep_on(&pageoutd_waitq)) {
                    dbg(DBG_PRINT, "Yeah, I am out, babe!\n");
                        kthread_exit((void *)0);
                }
