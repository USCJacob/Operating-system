...
special_file_mmap(vnode_t *file, vmarea_t *vma, mmobj_t **ret)
{
    ...
    KASSERT(file && file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->mmap);
...
special_file_fillpage(vnode_t *file, off_t offset, void *pagebuf)
{
    ...
    KASSERT(file && file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->fillpage);
...
special_file_dirtypage(vnode_t *file, off_t offset)
{
    ...
    KASSERT(file && file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->dirtypage);
...
special_file_cleanpage(vnode_t *file, off_t offset, void *pagebuf)
{
    ...
    KASSERT(file && file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->cleanpage);
