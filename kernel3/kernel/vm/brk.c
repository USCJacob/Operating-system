...
do_brk(void *addr, void **ret)
{
        ...
        if((unsigned int)addr < (unsigned int)curproc->p_start_brk) {
                dbg(DBG_PRINT,"BRK 2\n");
            	return -ENOMEM;
        }
        ...
        //Page after to the page with addr
        int endvfn = ADDR_TO_PN((uint32_t)addr - 1) + 1; 

        //Page after to the page with p_brk
        int startvfn = ADDR_TO_PN((uint32_t)curproc->p_brk - 1) + 1; 
        
        vmmap_t *vmap = curproc->p_vmmap;

        if(endvfn < startvfn){
            dbg(DBG_PRINT, "BRK 4)\n");
            vmmap_remove(vmap, endvfn, (startvfn - endvfn));
	            ...
	            vmarea_t *vmarea = vmmap_lookup(vmap, startvfn-1);
	            if(vmmap_is_range_empty(vmap, startvfn, (endvfn - startvfn)) == 0) { 
                                dbg(DBG_PRINT,"BRK 6 \n");
                                return -ENOMEM;
                }  
                 dbg(DBG_PRINT,"BRK 7 \n");
