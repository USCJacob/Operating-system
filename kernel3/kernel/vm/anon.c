...
anon_put(mmobj_t *o)
{
        ...
        KASSERT(o && (0 < o->mmo_refcount) && (&anon_mmobj_ops == o->mmo_ops));
        dbg(DBG_PRINT,"(ANON PUT)\n");
        
        if(o->mmo_refcount-1 == o->mmo_nrespages){
                pframe_t *f;
                 dbg(DBG_PRINT,"(ANON PUT)\n");
                 
                list_iterate_begin(&o->mmo_respages, f, pframe_t, pf_olink){
                        dbg(DBG_PRINT,"(ANON PUT)\n");
                        
                        if(pframe_is_pinned(f)){
                                dbg(DBG_PRINT,"(ANON PUT)\n");
                                pframe_unpin(f);
                                dbg(DBG_PRINT,"(ANON PUT)\n");
                        }
...
anon_lookuppage(mmobj_t *o, uint32_t pagenum, int forwrite, pframe_t **pf)
{
        ...
        //return -1;
        int p = pframe_get(o, pagenum, pf);
        dbg(DBG_PRINT, "(ANON LOOKUPPAGE)\n");
        return p;
}
