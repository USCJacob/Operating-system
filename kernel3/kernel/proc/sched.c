...
int sched_cancellable_sleep_on(ktqueue_t *q)
{
        ...
        //  replace 0 with 1 as a stub to make pageoutd to exit
        // return 1;
        if (curthr->kt_cancelled != 1)
        {
                ...
                {
                    dbg(DBG_PRINT, "I am canceled, babe!\n");
                        return -EINTR;
                }
        }
...
void sched_switch(void)
{
        ...
        dbg(DBG_PRINT, "runnable queue size : %d\n", (kt_runq.tq_size));

        // old thread
        kthread_t *thr;

        uint8_t cIpl = intr_getipl();
        // mask intr
        ...
        dbg(DBG_PRINT, "next pid in runnable queue: %d\n", curproc->p_pid);
        context_switch(&thr->kt_ctx, &curthr->kt_ctx);
        intr_setipl(cIpl);
