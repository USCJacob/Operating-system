...
sys_read(read_args_t *arg)
{
    ...
    read_args_t kern_arg;
    int         err;
    void*       page_addr;
    int         bytes_read;
    
    // - copy_from_user() the read_args_t
    if ((err = copy_from_user(&kern_arg, arg, sizeof(read_args_t))) < 0) {
        curthr->kt_errno = -err;
        return -1;
    }
    
    // - copy_from_user() the read_args_t
    page_addr = page_alloc();
    if (NULL == page_addr) {
        // run out of memory
    ...
    // call do_read(),
    bytes_read = do_read(kern_arg.fd, page_addr, kern_arg.nbytes);
    if (bytes_read < 0) {
    ...
    // and copy_to_user() the read bytes
    err = copy_to_user(kern_arg.buf, page_addr, bytes_read);
    if (err != 0) {
        curthr->kt_errno = -err;
        page_free(page_addr);
        return -1;
...
sys_write(write_args_t *arg)
{
    ...
    write_args_t    kern_arg;
    int             err;
    void*           page_addr;
    int             bytes_write;
    
    // - copy_from_user() the read_args_t
    if ((err = copy_from_user(&kern_arg, arg, sizeof(write_args_t))) < 0) {
        curthr->kt_errno = -err;
        return -1;
    }
    ...
    // - copy from user the write content
    if ((err = copy_from_user(page_addr, kern_arg.buf, kern_arg.nbytes)) < 0) {
        curthr->kt_errno = -err;
        page_free(page_addr);
        return -1;
    }
    ...
    // do write
    bytes_write = do_write(kern_arg.fd, page_addr, kern_arg.nbytes);
    if (bytes_write < 0) {
...
sys_getdents(getdents_args_t *arg)
{
    ...
    getdents_args_t     kern_arg;
    dirent_t            kern_dirent;
    int                 err;
    
    // copy from user for getdents_args_t
    if ((err = copy_from_user(&kern_arg, arg, sizeof(getdents_args_t))) < 0) {
        curthr->kt_errno = -err;
        return -1;
    }
    
    /** call do_getdent in a
      * loop until you have read getdent_args_t->count bytes (or an error
      * occurs)
      */
    for (size_t i = 0; i <= (kern_arg.count / sizeof(dirent_t)); i++) {
        err = do_getdent(kern_arg.fd, &kern_dirent);
        if (err < 0) {
            curthr->kt_errno = -err;
            return -1;
        }
        err = copy_to_user(kern_arg.dirp, &kern_dirent, sizeof(dirent_t));
        if (err != 0) {
            curthr->kt_errno = -err;
            return -1;
        }
