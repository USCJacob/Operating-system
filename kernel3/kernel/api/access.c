...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
    ...
    vmmap_t* mp = p ->p_vmmap;
    uint32_t pn = ADDR_TO_PN(vaddr);
    vmarea_t* vma = vmmap_lookup(mp, pn);
    if (NULL != vma) {
        int vma_perm = vma->vma_prot;
        int combo = (PROT_READ | PROT_WRITE | PROT_EXEC);
        return ((vma_perm & combo) == (perm & combo) ? 1 : 0);
    }
    return 0;
}
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
    ...
    for (size_t i = 0; i < len; i++) {
        size_t addr = (size_t)avaddr + i;
        if (0 == addr_perm(p, (void*)addr, perm)) {
            return 0;
        }
    }
    return 1;
